...
const ShopCheckout = () => {
  ...
  // const cartItems = location.state.cartItems.cartItems || [];
  ...
  const placeOrder = async () => {
    try {
      ...
      const orderCodeResponse = await api.post("/commonApi/getCodeValue", {
        type: "orders",
      });
      ...
      const invoiceCodeResponse = await api.post("/commonApi/getCodeValue", {
        type: "invoice",
      });

      if (
        invoiceCodeResponse.status !== 200 ||
        !invoiceCodeResponse.data.data
      ) {
      ...
      if (
        ...
        !invoiceResponse.data.data.insertId
      ) {
